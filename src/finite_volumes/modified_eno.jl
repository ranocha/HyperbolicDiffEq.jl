...
@inline function (::MinL2Choice)(u_m1, u_0, u_p1)
    idx = -1
    val_old = 0.08333333333333333*(-u_m1 + u_0)^2 # + 1.0*u_0^2

    val_new = 0.08333333333333333*(-u_0 + u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m2, u_m1, u_0, u_p1, u_p2)
    idx = -2
    val_old = 0.001388888888888889*(-2*u_m1 + u_m2 + u_0)^2 + 0.020833333333333332*(-4*u_m1 + u_m2 + 3*u_0)^2 # + 1.0*u_0^2

    val_new = 0.001388888888888889*(u_m1 - 2*u_0 + u_p1)^2 + 0.020833333333333332*(-u_m1 + u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 0.001388888888888889*(u_0 - 2*u_p1 + u_p2)^2 + 0.020833333333333332*(-3*u_0 + 4*u_p1 - u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m3, u_m2, u_m1, u_0, u_p1, u_p2, u_p3)
    idx = -3
    val_old = 9.92063492063492e-6*(-3*u_m1 + 3*u_m2 - u_m3 + u_0)^2 + 0.001388888888888889*(-5*u_m1 + 4*u_m2 - u_m3 + 2*u_0)^2 + 2.3148148148148147e-5*(-177*u_m1 + 87*u_m2 - 19*u_m3 + 109*u_0)^2 # + 1.0*u_0^2

    val_new = 9.92063492063492e-6*(3*u_m1 - u_m2 - 3*u_0 + u_p1)^2 + 0.001388888888888889*(u_m1 - 2*u_0 + u_p1)^2 + 2.3148148148148147e-5*(-63*u_m1 + 11*u_m2 + 33*u_0 + 19*u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -2
        val_old = val_new
    end

    val_new = 9.92063492063492e-6*(-u_m1 + 3*u_0 - 3*u_p1 + u_p2)^2 + 0.001388888888888889*(u_m1 - 2*u_0 + u_p1)^2 + 2.3148148148148147e-5*(-19*u_m1 - 33*u_0 + 63*u_p1 - 11*u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 9.92063492063492e-6*(-u_0 + 3*u_p1 - 3*u_p2 + u_p3)^2 + 0.001388888888888889*(2*u_0 - 5*u_p1 + 4*u_p2 - u_p3)^2 + 2.3148148148148147e-5*(-109*u_0 + 177*u_p1 - 87*u_p2 + 19*u_p3)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m4, u_m3, u_m2, u_m1, u_0, u_p1, u_p2, u_p3, u_p4)
    idx = -4
    val_old = 3.936759889140842e-8*(-4*u_m1 + 6*u_m2 - 4*u_m3 + u_m4 + u_0)^2 + 2.48015873015873e-6*(-18*u_m1 + 24*u_m2 - 14*u_m3 + 3*u_m4 + 5*u_0)^2 + 1.7715419501133788e-6*(-240*u_m1 + 262*u_m2 - 128*u_m3 + 25*u_m4 + 81*u_0)^2 + 5.787037037037037e-6*(-462*u_m1 + 336*u_m2 - 146*u_m3 + 27*u_m4 + 245*u_0)^2 # + 1.0*u_0^2

    val_new = 3.936759889140842e-8*(6*u_m1 - 4*u_m2 + u_m3 - 4*u_0 + u_p1)^2 + 2.48015873015873e-6*(12*u_m1 - 6*u_m2 + u_m3 - 10*u_0 + 3*u_p1)^2 + 1.7715419501133788e-6*(10*u_m1 + 12*u_m2 - 3*u_m3 - 44*u_0 + 25*u_p1)^2 + 5.787037037037037e-6*(-192*u_m1 + 66*u_m2 - 11*u_m3 + 110*u_0 + 27*u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -3
        val_old = val_new
    end

    val_new = 3.936759889140842e-8*(-4*u_m1 + u_m2 + 6*u_0 - 4*u_p1 + u_p2)^2 + 2.48015873015873e-6*(2*u_m1 - u_m2 - 2*u_p1 + u_p2)^2 + 1.7715419501133788e-6*(40*u_m1 - 3*u_m2 - 74*u_0 + 40*u_p1 - 3*u_p2)^2 + 5.787037037037037e-6*(-82*u_m1 + 11*u_m2 + 82*u_p1 - 11*u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -2
        val_old = val_new
    end

    val_new = 3.936759889140842e-8*(u_m1 - 4*u_0 + 6*u_p1 - 4*u_p2 + u_p3)^2 + 2.48015873015873e-6*(-3*u_m1 + 10*u_0 - 12*u_p1 + 6*u_p2 - u_p3)^2 + 1.7715419501133788e-6*(25*u_m1 - 44*u_0 + 10*u_p1 + 12*u_p2 - 3*u_p3)^2 + 5.787037037037037e-6*(-27*u_m1 - 110*u_0 + 192*u_p1 - 66*u_p2 + 11*u_p3)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 3.936759889140842e-8*(u_0 - 4*u_p1 + 6*u_p2 - 4*u_p3 + u_p4)^2 + 2.48015873015873e-6*(-5*u_0 + 18*u_p1 - 24*u_p2 + 14*u_p3 - 3*u_p4)^2 + 1.7715419501133788e-6*(81*u_0 - 240*u_p1 + 262*u_p2 - 128*u_p3 + 25*u_p4)^2 + 5.787037037037037e-6*(-245*u_0 + 462*u_p1 - 336*u_p2 + 146*u_p3 - 27*u_p4)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m5, u_m4, u_m3, u_m2, u_m1, u_0, u_p1, u_p2, u_p3, u_p4, u_p5)
    idx = -5
    val_old = 9.941312851365762e-11*(-5*u_m1 + 10*u_m2 - 10*u_m3 + 5*u_m4 - u_m5 + u_0)^2 + 3.936759889140842e-8*(-14*u_m1 + 26*u_m2 - 24*u_m3 + 11*u_m4 - 2*u_m5 + 3*u_0)^2 + 3.061924358220655e-8*(-317*u_m1 + 526*u_m2 - 436*u_m3 + 182*u_m4 - 31*u_m5 + 76*u_0)^2 + 1.7715419501133788e-6*(-350*u_m1 + 482*u_m2 - 348*u_m3 + 135*u_m4 - 22*u_m5 + 103*u_0)^2 + 3.2806332409507015e-9*(-23719*u_m1 + 22742*u_m2 - 14762*u_m3 + 5449*u_m4 - 863*u_m5 + 11153*u_0)^2 # + 1.0*u_0^2

    val_new = 9.941312851365762e-11*(10*u_m1 - 10*u_m2 + 5*u_m3 - u_m4 - 5*u_0 + u_p1)^2 + 3.936759889140842e-8*(16*u_m1 - 14*u_m2 + 6*u_m3 - u_m4 - 9*u_0 + 2*u_p1)^2 + 3.061924358220655e-8*(148*u_m1 - 94*u_m2 + 29*u_m3 - 4*u_m4 - 110*u_0 + 31*u_p1)^2 + 1.7715419501133788e-6*(-20*u_m1 + 42*u_m2 - 18*u_m3 + 3*u_m4 - 29*u_0 + 22*u_p1)^2 + 3.2806332409507015e-9*(-10774*u_m1 + 5482*u_m2 - 1817*u_m3 + 271*u_m4 + 5975*u_0 + 863*u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -4
        val_old = val_new
    end

    val_new = 9.941312851365762e-11*(-10*u_m1 + 5*u_m2 - u_m3 + 10*u_0 - 5*u_p1 + u_p2)^2 + 3.936759889140842e-8*(-4*u_m1 + u_m2 + 6*u_0 - 4*u_p1 + u_p2)^2 + 3.061924358220655e-8*(68*u_m1 - 34*u_m2 + 5*u_m3 - 50*u_0 + 7*u_p1 + 4*u_p2)^2 + 1.7715419501133788e-6*(40*u_m1 - 3*u_m2 - 74*u_0 + 40*u_p1 - 3*u_p2)^2 + 3.2806332409507015e-9*(-5354*u_m1 + 1417*u_m2 - 191*u_m3 + 1910*u_0 + 2489*u_p1 - 271*u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -3
        val_old = val_new
    end

    val_new = 9.941312851365762e-11*(5*u_m1 - u_m2 - 10*u_0 + 10*u_p1 - 5*u_p2 + u_p3)^2 + 3.936759889140842e-8*(-4*u_m1 + u_m2 + 6*u_0 - 4*u_p1 + u_p2)^2 + 3.061924358220655e-8*(-7*u_m1 - 4*u_m2 + 50*u_0 - 68*u_p1 + 34*u_p2 - 5*u_p3)^2 + 1.7715419501133788e-6*(40*u_m1 - 3*u_m2 - 74*u_0 + 40*u_p1 - 3*u_p2)^2 + 3.2806332409507015e-9*(-2489*u_m1 + 271*u_m2 - 1910*u_0 + 5354*u_p1 - 1417*u_p2 + 191*u_p3)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -2
        val_old = val_new
    end

    val_new = 9.941312851365762e-11*(-u_m1 + 5*u_0 - 10*u_p1 + 10*u_p2 - 5*u_p3 + u_p4)^2 + 3.936759889140842e-8*(2*u_m1 - 9*u_0 + 16*u_p1 - 14*u_p2 + 6*u_p3 - u_p4)^2 + 3.061924358220655e-8*(-31*u_m1 + 110*u_0 - 148*u_p1 + 94*u_p2 - 29*u_p3 + 4*u_p4)^2 + 1.7715419501133788e-6*(22*u_m1 - 29*u_0 - 20*u_p1 + 42*u_p2 - 18*u_p3 + 3*u_p4)^2 + 3.2806332409507015e-9*(-863*u_m1 - 5975*u_0 + 10774*u_p1 - 5482*u_p2 + 1817*u_p3 - 271*u_p4)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 9.941312851365762e-11*(-u_0 + 5*u_p1 - 10*u_p2 + 10*u_p3 - 5*u_p4 + u_p5)^2 + 3.936759889140842e-8*(3*u_0 - 14*u_p1 + 26*u_p2 - 24*u_p3 + 11*u_p4 - 2*u_p5)^2 + 3.061924358220655e-8*(-76*u_0 + 317*u_p1 - 526*u_p2 + 436*u_p3 - 182*u_p4 + 31*u_p5)^2 + 1.7715419501133788e-6*(103*u_0 - 350*u_p1 + 482*u_p2 - 348*u_p3 + 135*u_p4 - 22*u_p5)^2 + 3.2806332409507015e-9*(-11153*u_0 + 23719*u_p1 - 22742*u_p2 + 14762*u_p3 - 5449*u_p4 + 863*u_p5)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m6, u_m5, u_m4, u_m3, u_m2, u_m1, u_0, u_p1, u_p2, u_p3, u_p4, u_p5, u_p6)
    idx = -6
    val_old = 1.7379917572317765e-13*(-6*u_m1 + 15*u_m2 - 20*u_m3 + 15*u_m4 - 6*u_m5 + u_m6 + u_0)^2 + 2.4853282128414405e-11*(-40*u_m1 + 95*u_m2 - 120*u_m3 + 85*u_m4 - 32*u_m5 + 5*u_m6 + 7*u_0)^2 + 9.037557137605237e-12*(-2034*u_m1 + 4491*u_m2 - 5284*u_m3 + 3501*u_m4 - 1242*u_m5 + 185*u_m6 + 383*u_0)^2 + 7.654810895551637e-9*(-1024*u_m1 + 2027*u_m2 - 2172*u_m3 + 1339*u_m4 - 452*u_m5 + 65*u_m6 + 217*u_0)^2 + 5.467722068251169e-11*(-83994*u_m1 + 139245*u_m2 - 132620*u_m3 + 76785*u_m4 - 24954*u_m5 + 3499*u_m6 + 22039*u_0)^2 + 8.201583102376754e-10*(-55688*u_m1 + 66109*u_m2 - 57024*u_m3 + 31523*u_m4 - 9976*u_m5 + 1375*u_m6 + 23681*u_0)^2 # + 1.0*u_0^2

    val_new = 1.7379917572317765e-13*(15*u_m1 - 20*u_m2 + 15*u_m3 - 6*u_m4 + u_m5 - 6*u_0 + u_p1)^2 + 2.4853282128414405e-11*(65*u_m1 - 80*u_m2 + 55*u_m3 - 20*u_m4 + 3*u_m5 - 28*u_0 + 5*u_p1)^2 + 9.037557137605237e-12*(1851*u_m1 - 1984*u_m2 + 1191*u_m3 - 384*u_m4 + 53*u_m5 - 912*u_0 + 185*u_p1)^2 + 7.654810895551637e-9*(341*u_m1 - 248*u_m2 + 103*u_m3 - 26*u_m4 + 3*u_m5 - 238*u_0 + 65*u_p1)^2 + 5.467722068251169e-11*(-10515*u_m1 + 16780*u_m2 - 10155*u_m3 + 3306*u_m4 - 461*u_m5 - 2454*u_0 + 3499*u_p1)^2 + 8.201583102376754e-10*(-26813*u_m1 + 17984*u_m2 - 8899*u_m3 + 2648*u_m4 - 351*u_m5 + 14056*u_0 + 1375*u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -5
        val_old = val_new
    end

    val_new = 1.7379917572317765e-13*(-20*u_m1 + 15*u_m2 - 6*u_m3 + u_m4 + 15*u_0 - 6*u_p1 + u_p2)^2 + 2.4853282128414405e-11*(-40*u_m1 + 25*u_m2 - 8*u_m3 + u_m4 + 35*u_0 - 16*u_p1 + 3*u_p2)^2 + 9.037557137605237e-12*(-4*u_m1 - 129*u_m2 + 78*u_m3 - 13*u_m4 + 201*u_0 - 186*u_p1 + 53*u_p2)^2 + 7.654810895551637e-9*(236*u_m1 - 143*u_m2 + 40*u_m3 - 5*u_m4 - 175*u_0 + 44*u_p1 + 3*u_p2)^2 + 5.467722068251169e-11*(5620*u_m1 + 645*u_m2 - 474*u_m3 + 79*u_m4 - 12135*u_0 + 6726*u_p1 - 461*u_p2)^2 + 8.201583102376754e-10*(-14528*u_m1 + 5699*u_m2 - 1528*u_m3 + 191*u_m4 + 6685*u_0 + 3832*u_p1 - 351*u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -4
        val_old = val_new
    end

    val_new = 1.7379917572317765e-13*(15*u_m1 - 6*u_m2 + u_m3 - 20*u_0 + 15*u_p1 - 6*u_p2 + u_p3)^2 + 2.4853282128414405e-11*(-5*u_m1 + 4*u_m2 - u_m3 + 5*u_p1 - 4*u_p2 + u_p3)^2 + 9.037557137605237e-12*(-459*u_m1 + 144*u_m2 - 13*u_m3 + 656*u_0 - 459*u_p1 + 144*u_p2 - 13*u_p3)^2 + 7.654810895551637e-9*(61*u_m1 - 38*u_m2 + 5*u_m3 - 61*u_p1 + 38*u_p2 - 5*u_p3)^2 + 5.467722068251169e-11*(8385*u_m1 - 1014*u_m2 + 79*u_m3 - 14900*u_0 + 8385*u_p1 - 1014*u_p2 + 79*u_p3)^2 + 8.201583102376754e-10*(-7843*u_m1 + 1688*u_m2 - 191*u_m3 + 7843*u_p1 - 1688*u_p2 + 191*u_p3)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -3
        val_old = val_new
    end

    val_new = 1.7379917572317765e-13*(-6*u_m1 + u_m2 + 15*u_0 - 20*u_p1 + 15*u_p2 - 6*u_p3 + u_p4)^2 + 2.4853282128414405e-11*(16*u_m1 - 3*u_m2 - 35*u_0 + 40*u_p1 - 25*u_p2 + 8*u_p3 - u_p4)^2 + 9.037557137605237e-12*(-186*u_m1 + 53*u_m2 + 201*u_0 - 4*u_p1 - 129*u_p2 + 78*u_p3 - 13*u_p4)^2 + 7.654810895551637e-9*(-44*u_m1 - 3*u_m2 + 175*u_0 - 236*u_p1 + 143*u_p2 - 40*u_p3 + 5*u_p4)^2 + 5.467722068251169e-11*(6726*u_m1 - 461*u_m2 - 12135*u_0 + 5620*u_p1 + 645*u_p2 - 474*u_p3 + 79*u_p4)^2 + 8.201583102376754e-10*(-3832*u_m1 + 351*u_m2 - 6685*u_0 + 14528*u_p1 - 5699*u_p2 + 1528*u_p3 - 191*u_p4)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -2
        val_old = val_new
    end

    val_new = 1.7379917572317765e-13*(u_m1 - 6*u_0 + 15*u_p1 - 20*u_p2 + 15*u_p3 - 6*u_p4 + u_p5)^2 + 2.4853282128414405e-11*(-5*u_m1 + 28*u_0 - 65*u_p1 + 80*u_p2 - 55*u_p3 + 20*u_p4 - 3*u_p5)^2 + 9.037557137605237e-12*(185*u_m1 - 912*u_0 + 1851*u_p1 - 1984*u_p2 + 1191*u_p3 - 384*u_p4 + 53*u_p5)^2 + 7.654810895551637e-9*(-65*u_m1 + 238*u_0 - 341*u_p1 + 248*u_p2 - 103*u_p3 + 26*u_p4 - 3*u_p5)^2 + 5.467722068251169e-11*(3499*u_m1 - 2454*u_0 - 10515*u_p1 + 16780*u_p2 - 10155*u_p3 + 3306*u_p4 - 461*u_p5)^2 + 8.201583102376754e-10*(-1375*u_m1 - 14056*u_0 + 26813*u_p1 - 17984*u_p2 + 8899*u_p3 - 2648*u_p4 + 351*u_p5)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 1.7379917572317765e-13*(u_0 - 6*u_p1 + 15*u_p2 - 20*u_p3 + 15*u_p4 - 6*u_p5 + u_p6)^2 + 2.4853282128414405e-11*(-7*u_0 + 40*u_p1 - 95*u_p2 + 120*u_p3 - 85*u_p4 + 32*u_p5 - 5*u_p6)^2 + 9.037557137605237e-12*(383*u_0 - 2034*u_p1 + 4491*u_p2 - 5284*u_p3 + 3501*u_p4 - 1242*u_p5 + 185*u_p6)^2 + 7.654810895551637e-9*(-217*u_0 + 1024*u_p1 - 2027*u_p2 + 2172*u_p3 - 1339*u_p4 + 452*u_p5 - 65*u_p6)^2 + 5.467722068251169e-11*(22039*u_0 - 83994*u_p1 + 139245*u_p2 - 132620*u_p3 + 76785*u_p4 - 24954*u_p5 + 3499*u_p6)^2 + 8.201583102376754e-10*(-23681*u_0 + 55688*u_p1 - 66109*u_p2 + 57024*u_p3 - 31523*u_p4 + 9976*u_p5 - 1375*u_p6)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
...
@inline function (::MinL2Choice)(u_m7, u_m6, u_m5, u_m4, u_m3, u_m2, u_m1, u_0, u_p1, u_p2, u_p3, u_p4, u_p5, u_p6, u_p7)
    idx = -7
    val_old = 2.2281945605535596e-16*(-7*u_m1 + 21*u_m2 - 35*u_m3 + 35*u_m4 - 21*u_m5 + 7*u_m6 - u_m7 + u_0)^2 + 1.7379917572317765e-13*(-27*u_m1 + 78*u_m2 - 125*u_m3 + 120*u_m4 - 69*u_m5 + 22*u_m6 - 3*u_m7 + 4*u_0)^2 + 3.6765210249133736e-14*(-2545*u_m1 + 6985*u_m2 - 10645*u_m3 + 9735*u_m4 - 5347*u_m5 + 1635*u_m6 - 215*u_m7 + 397*u_0)^2 + 9.037557137605237e-12*(-3609*u_m1 + 9216*u_m2 - 13159*u_m3 + 11376*u_m4 - 5967*u_m5 + 1760*u_m6 - 225*u_m7 + 608*u_0)^2 + 1.5815724990809165e-13*(-326087*u_m1 + 748361*u_m2 - 981875*u_m3 + 798615*u_m4 - 401861*u_m5 + 115107*u_m6 - 14401*u_m7 + 62141*u_0)^2 + 5.467722068251169e-11*(-105813*u_m1 + 204702*u_m2 - 241715*u_m3 + 185880*u_m4 - 90411*u_m5 + 25318*u_m6 - 3117*u_m7 + 25156*u_0)^2 + 9.112870113751949e-13*(-1908311*u_m1 + 2696283*u_m2 - 2899075*u_m3 + 2134045*u_m4 - 1012293*u_m5 + 278921*u_m6 - 33953*u_m7 + 744383*u_0)^2 # + 1.0*u_0^2

    val_new = 2.2281945605535596e-16*(21*u_m1 - 35*u_m2 + 35*u_m3 - 21*u_m4 + 7*u_m5 - u_m6 - 7*u_0 + u_p1)^2 + 1.7379917572317765e-13*(57*u_m1 - 90*u_m2 + 85*u_m3 - 48*u_m4 + 15*u_m5 - 2*u_m6 - 20*u_0 + 3*u_p1)^2 + 3.6765210249133736e-14*(3475*u_m1 - 5055*u_m2 + 4405*u_m3 - 2305*u_m4 + 673*u_m5 - 85*u_m6 - 1323*u_0 + 215*u_p1)^2 + 9.037557137605237e-12*(2691*u_m1 - 3384*u_m2 + 2591*u_m3 - 1224*u_m4 + 333*u_m5 - 40*u_m6 - 1192*u_0 + 225*u_p1)^2 + 1.5815724990809165e-13*(77141*u_m1 - 58095*u_m2 + 26195*u_m3 - 7841*u_m4 + 1367*u_m5 - 101*u_m6 - 53067*u_0 + 14401*u_p1)^2 + 5.467722068251169e-11*(-18537*u_m1 + 30150*u_m2 - 23525*u_m3 + 11328*u_m4 - 3135*u_m5 + 382*u_m6 + 220*u_0 + 3117*u_p1)^2 + 9.112870113751949e-13*(-957627*u_m1 + 794915*u_m2 - 522365*u_m3 + 232677*u_m4 - 61609*u_m5 + 7297*u_m6 + 472759*u_0 + 33953*u_p1)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -6
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(-35*u_m1 + 35*u_m2 - 21*u_m3 + 7*u_m4 - u_m5 + 21*u_0 - 7*u_p1 + u_p2)^2 + 1.7379917572317765e-13*(-55*u_m1 + 50*u_m2 - 27*u_m3 + 8*u_m4 - u_m5 + 36*u_0 - 13*u_p1 + 2*u_p2)^2 + 3.6765210249133736e-14*(-1285*u_m1 + 895*u_m2 - 355*u_m3 + 75*u_m4 - 7*u_m5 + 1057*u_0 - 465*u_p1 + 85*u_p2)^2 + 9.037557137605237e-12*(451*u_m1 - 584*u_m2 + 351*u_m3 - 104*u_m4 + 13*u_m5 - 72*u_0 - 95*u_p1 + 40*u_p2)^2 + 1.5815724990809165e-13*(71485*u_m1 - 51025*u_m2 + 20539*u_m3 - 5013*u_m4 + 559*u_m5 - 50239*u_0 + 13593*u_p1 + 101*u_p2)^2 + 5.467722068251169e-11*(2855*u_m1 + 3410*u_m2 - 2133*u_m3 + 632*u_m4 - 79*u_m5 - 10476*u_0 + 6173*u_p1 - 382*u_p2)^2 + 9.112870113751949e-13*(-548995*u_m1 + 284125*u_m2 - 113733*u_m3 + 28361*u_m4 - 3233*u_m5 + 268443*u_0 + 92329*u_p1 - 7297*u_p2)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -5
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(35*u_m1 - 21*u_m2 + 7*u_m3 - u_m4 - 35*u_0 + 21*u_p1 - 7*u_p2 + u_p3)^2 + 1.7379917572317765e-13*(15*u_m1 - 6*u_m2 + u_m3 - 20*u_0 + 15*u_p1 - 6*u_p2 + u_p3)^2 + 3.6765210249133736e-14*(-795*u_m1 + 503*u_m2 - 159*u_m3 + 19*u_m4 + 665*u_0 - 269*u_p1 + 29*u_p2 + 7*u_p3)^2 + 9.037557137605237e-12*(-459*u_m1 + 144*u_m2 - 13*u_m3 + 656*u_0 - 459*u_p1 + 144*u_p2 - 13*u_p3)^2 + 1.5815724990809165e-13*(32355*u_m1 - 19721*u_m2 + 4887*u_m3 - 541*u_m4 - 18935*u_0 - 2059*u_p1 + 4573*u_p2 - 559*u_p3)^2 + 5.467722068251169e-11*(8385*u_m1 - 1014*u_m2 + 79*u_m3 - 14900*u_0 + 8385*u_p1 - 1014*u_p2 + 79*u_p3)^2 + 9.112870113751949e-13*(-322685*u_m1 + 103077*u_m2 - 23209*u_m3 + 2497*u_m4 + 87395*u_0 + 182853*u_p1 - 33161*u_p2 + 3233*u_p3)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -4
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(-21*u_m1 + 7*u_m2 - u_m3 + 35*u_0 - 35*u_p1 + 21*u_p2 - 7*u_p3 + u_p4)^2 + 1.7379917572317765e-13*(15*u_m1 - 6*u_m2 + u_m3 - 20*u_0 + 15*u_p1 - 6*u_p2 + u_p3)^2 + 3.6765210249133736e-14*(269*u_m1 - 29*u_m2 - 7*u_m3 - 665*u_0 + 795*u_p1 - 503*u_p2 + 159*u_p3 - 19*u_p4)^2 + 9.037557137605237e-12*(-459*u_m1 + 144*u_m2 - 13*u_m3 + 656*u_0 - 459*u_p1 + 144*u_p2 - 13*u_p3)^2 + 1.5815724990809165e-13*(2059*u_m1 - 4573*u_m2 + 559*u_m3 + 18935*u_0 - 32355*u_p1 + 19721*u_p2 - 4887*u_p3 + 541*u_p4)^2 + 5.467722068251169e-11*(8385*u_m1 - 1014*u_m2 + 79*u_m3 - 14900*u_0 + 8385*u_p1 - 1014*u_p2 + 79*u_p3)^2 + 9.112870113751949e-13*(-182853*u_m1 + 33161*u_m2 - 3233*u_m3 - 87395*u_0 + 322685*u_p1 - 103077*u_p2 + 23209*u_p3 - 2497*u_p4)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -3
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(7*u_m1 - u_m2 - 21*u_0 + 35*u_p1 - 35*u_p2 + 21*u_p3 - 7*u_p4 + u_p5)^2 + 1.7379917572317765e-13*(-13*u_m1 + 2*u_m2 + 36*u_0 - 55*u_p1 + 50*u_p2 - 27*u_p3 + 8*u_p4 - u_p5)^2 + 3.6765210249133736e-14*(465*u_m1 - 85*u_m2 - 1057*u_0 + 1285*u_p1 - 895*u_p2 + 355*u_p3 - 75*u_p4 + 7*u_p5)^2 + 9.037557137605237e-12*(-95*u_m1 + 40*u_m2 - 72*u_0 + 451*u_p1 - 584*u_p2 + 351*u_p3 - 104*u_p4 + 13*u_p5)^2 + 1.5815724990809165e-13*(-13593*u_m1 - 101*u_m2 + 50239*u_0 - 71485*u_p1 + 51025*u_p2 - 20539*u_p3 + 5013*u_p4 - 559*u_p5)^2 + 5.467722068251169e-11*(6173*u_m1 - 382*u_m2 - 10476*u_0 + 2855*u_p1 + 3410*u_p2 - 2133*u_p3 + 632*u_p4 - 79*u_p5)^2 + 9.112870113751949e-13*(-92329*u_m1 + 7297*u_m2 - 268443*u_0 + 548995*u_p1 - 284125*u_p2 + 113733*u_p3 - 28361*u_p4 + 3233*u_p5)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -2
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(-u_m1 + 7*u_0 - 21*u_p1 + 35*u_p2 - 35*u_p3 + 21*u_p4 - 7*u_p5 + u_p6)^2 + 1.7379917572317765e-13*(3*u_m1 - 20*u_0 + 57*u_p1 - 90*u_p2 + 85*u_p3 - 48*u_p4 + 15*u_p5 - 2*u_p6)^2 + 3.6765210249133736e-14*(-215*u_m1 + 1323*u_0 - 3475*u_p1 + 5055*u_p2 - 4405*u_p3 + 2305*u_p4 - 673*u_p5 + 85*u_p6)^2 + 9.037557137605237e-12*(225*u_m1 - 1192*u_0 + 2691*u_p1 - 3384*u_p2 + 2591*u_p3 - 1224*u_p4 + 333*u_p5 - 40*u_p6)^2 + 1.5815724990809165e-13*(-14401*u_m1 + 53067*u_0 - 77141*u_p1 + 58095*u_p2 - 26195*u_p3 + 7841*u_p4 - 1367*u_p5 + 101*u_p6)^2 + 5.467722068251169e-11*(3117*u_m1 + 220*u_0 - 18537*u_p1 + 30150*u_p2 - 23525*u_p3 + 11328*u_p4 - 3135*u_p5 + 382*u_p6)^2 + 9.112870113751949e-13*(-33953*u_m1 - 472759*u_0 + 957627*u_p1 - 794915*u_p2 + 522365*u_p3 - 232677*u_p4 + 61609*u_p5 - 7297*u_p6)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = -1
        val_old = val_new
    end

    val_new = 2.2281945605535596e-16*(-u_0 + 7*u_p1 - 21*u_p2 + 35*u_p3 - 35*u_p4 + 21*u_p5 - 7*u_p6 + u_p7)^2 + 1.7379917572317765e-13*(4*u_0 - 27*u_p1 + 78*u_p2 - 125*u_p3 + 120*u_p4 - 69*u_p5 + 22*u_p6 - 3*u_p7)^2 + 3.6765210249133736e-14*(-397*u_0 + 2545*u_p1 - 6985*u_p2 + 10645*u_p3 - 9735*u_p4 + 5347*u_p5 - 1635*u_p6 + 215*u_p7)^2 + 9.037557137605237e-12*(608*u_0 - 3609*u_p1 + 9216*u_p2 - 13159*u_p3 + 11376*u_p4 - 5967*u_p5 + 1760*u_p6 - 225*u_p7)^2 + 1.5815724990809165e-13*(-62141*u_0 + 326087*u_p1 - 748361*u_p2 + 981875*u_p3 - 798615*u_p4 + 401861*u_p5 - 115107*u_p6 + 14401*u_p7)^2 + 5.467722068251169e-11*(25156*u_0 - 105813*u_p1 + 204702*u_p2 - 241715*u_p3 + 185880*u_p4 - 90411*u_p5 + 25318*u_p6 - 3117*u_p7)^2 + 9.112870113751949e-13*(-744383*u_0 + 1908311*u_p1 - 2696283*u_p2 + 2899075*u_p3 - 2134045*u_p4 + 1012293*u_p5 - 278921*u_p6 + 33953*u_p7)^2 # + 1.0*u_0^2
    if val_new < val_old
        idx = 0
        val_old = val_new
    end
